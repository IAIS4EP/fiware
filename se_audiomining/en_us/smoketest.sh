...
echo "Running smoke tests for the Audio Mining SE at $ROOT_URL_NO_SLASH"
...
CREATE_INDEX_RESULT=`curl -X POST -H "Content-Type:application/json" -H "Accept:application/json" -s -o /dev/null -w "%{http_code}" ${ROOT_URL_NO_SLASH}/?indexid=smoketest&language=English`
